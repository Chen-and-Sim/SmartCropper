...
wchar_t w_cur_name[200];
char cur_filename[100];
char cur_path[100];
char output_path[100] = "crops\\";
char output_name_format[100] = "%f-%s";
...
const int NUM = 19;
const char short_command[NUM][3] = {"-a", "-h", "-l", "-q", "-s", "-x", "-i", "-u", "-f", "-n", "-p", "-c", "-d", "-e", "-r", "-m", "-v", "-b", "-t"};
const char long_command[NUM][20] = {"about", "help",   "language", "quit",     "save",  "settings", "load",    "loadall",  "format", "name",
												"path",  "create", "delete",   "separate", "merge", "modify",   "preview", "setmerge", "tolerance"};
void (*functions[NUM]) (char*) = {about, help, switch_language, quit, save, current_settings, load_image, load_all, set_output_format, set_name_format,   
											 set_output_path, create_area, delete_area, separate_area, merge_area, modify_area, preview, set_merge, set_tolerance};
